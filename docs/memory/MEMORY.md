...
- 2026-06-12: Attachment click opens a preview modal instead of a new tab. Shared `components/ui/AttachmentPreview.svelte` (image=`<img>`, PDF=`<iframe>`, other=icon+download). Used by AttachmentsTab + task chips (Tasks + ProjectDetail). Footer: Deschide-in-tab-nou / Descarca (anchor with `download`) + optional Sterge (ondelete closes modal then runs parent confirm). CSP already allows same-origin img/iframe (`default-src 'self'` covers frame-src).
