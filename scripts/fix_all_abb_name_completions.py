"""Complete truncated descriere_scurta for ALL ABB families (ACS580 + ACS880)."""
...
for familie in ('ACS580', 'ACS880'):
    cur.execute("""
        ...
        WHERE familie=? AND descriere_scurta IS NOT NULL AND descriere_scurta != ''
    """, (familie,))
    rows = cur.fetchall()
    fixed = 0
    for param_id, parametru, name in rows:
        ...
        if new_name != name:
            ...
            fixed += 1
    print(f"{familie}: fixed {fixed} names")
