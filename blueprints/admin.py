...
# global search, dashboard home, and webhook deploy routes.
...
# ---------------------------------------------------------------------------
# Webhook deploy (NO @login_required -- uses HMAC verification)
# ---------------------------------------------------------------------------
...
@admin_bp.route('/webhook/deploy', methods=['POST'])
def webhook_deploy():
    ...
    # Go up one level from blueprints/ to the project root
        ...
        for pip_cmd in pip_cmds:
            try:
                ...
                if pip_result.returncode != 0:
                    ...
                    # nu blocam restart-ul -- incearca urmatorul fallback sau continua
                else:
