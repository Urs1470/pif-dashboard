...
export const plan = $state({
  ...
  backlog: [],
  start: '',
  // IMPLICIT: 14 pe DESKTOP (Ion, 2026-08-21: „pe desktop perioada by default
  // vreau sa fie cum a fost 14 zile, nu 30"), 30 pe TELEFON (grila de patru
  // saptamani cere 28 de zile, iar pe telefon bara de orizont nici nu se mai
  // arata). Se alege din latimea ecranului la initializare, ca sa nu existe un
  // dublu-load pe telefon. Efectul de plasa din Plan.svelte ramane.
  days: (typeof window !== 'undefined' && window.matchMedia
    && window.matchMedia('(max-width: 768px)').matches) ? 30 : 14,
  today: '',
  showDone: readDone(),
